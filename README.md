...
## Spent time
As requested in the task instructions, I am providing the time I spent on this task. I spent approximately 2 hours working on the core components and some tests for both APIs. It took an additional hour to add the remaining tests. Overall, the requested API was straightforward. However, the most challenging aspect of this task was figuring out how to provide the data as per the database structure and considering all edge cases. Therefore, it took me some time to update my queries and ensure that the data was provided accurately.
...
Currently, I'm using the above SQL query inside the Price model. However, I made some optimizations to improve the query's readability and functionality. (I haven't implemented the following query in my code yet.)
